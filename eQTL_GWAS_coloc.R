...
# ======================================
# Perform colocalisation analysis between Indonesian eQTLs and selected GWAS
# traits
# ======================================
...
for (trait in traits){
  ...
  for (gene in rownames(results)) {
    ...
    indoeQTL_nominal_egene <- indoeQTL_nominal[indoeQTL_nominal$target==gene , ]
    
    # Adding MAF
    indoeQTL_nominal_egene$variant_ID <- paste(indoeQTL_nominal_egene$chr, indoeQTL_nominal_egene$var_start, sep = "_")
    testVarInfo <- varInfo[varInfo$variant_ID %in% indoeQTL_nominal_egene$variant_ID , ]
    indoeQTL_nominal_egene <- indoeQTL_nominal_egene[indoeQTL_nominal_egene$variant_ID %in% testVarInfo$variant_ID , ]
    testVarInfo <- testVarInfo[match(indoeQTL_nominal_egene$variant_ID, testVarInfo$variant_ID) , ]
    
    indoeQTL_nominal_egene$maf <- testVarInfo$maf
    indoeQTL_nominal_egene$rsid <- as.character(indoeQTL_nominal_egene$rsid)
    
    trait_associations_sharedpos <- trait_associations[trait_associations$rsid %in% indoeQTL_nominal_egene$rsid , ]
    dim(trait_associations_sharedpos)
    
    # Merging input data for colocalization
    input <- merge(indoeQTL_nominal_egene, trait_associations_sharedpos, by="rsid", all = F,
